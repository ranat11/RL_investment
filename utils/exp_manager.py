...
class ExperimentManager(object):
    ...
    Please take a look at `rl_train.py` to have the details for each argument.
    """
    ...
    def __init__(
        ...
        self._is_trading = self.is_trading(env_id)
        ...
        self.tensorboard_log = None if tensorboard_log == "" else os.path.join(tensorboard_log, self.env_id) 
        ...
        self.save_path = os.path.join(
            self.log_path, f"{self.env_id}_{get_latest_run_id(self.log_path, self.env_id) + 1}"
        )
        self.params_path = f"{self.save_path}/parameters"
    ...
    def setup_experiment(self) -> Optional[BaseAlgorithm]:
        ...
        if self._is_trading:
            self.env_kwargs.update({"env_type" : "train"})
    ...
    def _preprocess_hyperparams(self, hyperparams: Dict[str, Any]) -> Tuple[Dict[str, Any], Optional[Callable], List[BaseCallback]]:
        ...
        if "window_size" in hyperparams.keys():
            self.env_kwargs.update({"window_size": hyperparams["window_size"]})
            del hyperparams["window_size"] 

        if "trade_time" in hyperparams.keys():
            self.env_kwargs.update({"trade_time": hyperparams["trade_time"]})
            del hyperparams["trade_time"] 
    ...
    def create_callbacks(self):
        ...
        if self.eval_freq > 0 and not self.optimize_hyperparameters:
            ...
            if self._is_trading:
                self.env_kwargs.update({"env_type" : "eval"})
    ...
    @staticmethod
    def is_trading(env_id: str) -> bool:
        # return env_id in ["ForexEnv-v0"]
        return "ForexEnv" in gym.envs.registry.env_specs[env_id].entry_point
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        if self._is_trading:
            sampled_env_hyperparams = HYPERPARAMS_SAMPLER[self.env_id](trial) 
            self.env_kwargs.update(sampled_env_hyperparams)
            self.env_kwargs.update({"env_type" : "train"})
